    ...
    async def execute(
        ...
        try:
            ...
            if check_path:
                ...
                if delete_objects:
                    await self.artist_image_repo.delete_by_artist_id(artist_id)
                    await self.artist_link_repo.delete_by_artist_id(artist_id)
                    status = await self.artist_repo.delete_by_id(artist_id)
            else:
                await self.artist_image_repo.delete_by_artist_id(artist_id)
                await self.artist_link_repo.delete_by_artist_id(artist_id)
                status = await self.artist_repo.delete_by_id(artist_id)
