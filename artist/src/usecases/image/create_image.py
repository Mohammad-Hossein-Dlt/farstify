...
class CreateImage:
    ...
    async def execute(
        ...
        try:
            ...
            artist: ArtistModel = await self.artist_repo.get_artist_by_id(to_create.artist_id)
            image: ArtistImageModel = ArtistImageModel.model_validate(to_create, from_attributes=True)
            ...
            if all([file, file_name, file_size, content_type]):
                ...
                try:
                    ...
                    if result:
                        ...
                        image: ArtistImageModel = await self.artist_image_repo.create_image(image)
                except:
                    if image.id:
                        await self.artist_image_repo.delete_image(image.id)
