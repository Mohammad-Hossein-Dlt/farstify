...
class ReorderImages:
    ...
    async def execute(
        ...
        try:
            images_list: list[ArtistImageModel] = await self.artist_image_repo.get_by_artist_id(artist_id)
            for index, image_id in enumerate(image_ids):
                for image in images_list:
                    if str(image.id) == image_id:
                        image.order = index
                        await self.artist_image_repo.update(image)

            return await self.artist_image_repo.get_by_artist_id(artist_id)
