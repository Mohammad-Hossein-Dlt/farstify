    ...
    async def execute(
        ...
        for category in categories:
            ...
            if category.parent_id:
                try:
                    parent: CategoryModel = await self.category_repo.get_by_id(category.parent_id)
                    category.parent_id = parent.id
                except EntityNotFoundError as ex:
                    raise EntityNotFoundError(ex.status_code, "Parent not found")
            
            try:
                category_model = CategoryModel.model_validate(category, from_attributes=True)
                await self.category_repo.create(category_model)
