...
class DeleteAllDocuments:
    ...
    async def execute(
        ...
        try:
            ...
            if status:
                await self.document_image_repo.delete_all_images()
                await self.document_link_repo.delete_all_links()
                status = await self.document_repo.delete_all_documents()                    
            
            return OperationOutput(id=None, request="delete/all-documents", status=status)
