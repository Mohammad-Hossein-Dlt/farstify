    ...
    async def execute(
        ...
        try:
            ...
            if check_path:
                ...
                if delete_objects:
                    await self.document_image_repo.delete_document_images(document_id)
                    await self.document_link_repo.delete_document_links(document_id)
                    status = await self.document_repo.delete_document(document_id)
            else:
                await self.document_image_repo.delete_document_images(document_id)
                await self.document_link_repo.delete_document_links(document_id)
                status = await self.document_repo.delete_document(document_id)
