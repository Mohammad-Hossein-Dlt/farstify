...
class CreateAllDocuments:
    ...
    async def execute(
        ...
        for document in documents:
            ...
            try:
                document_model = DocumentModel.model_validate(document, from_attributes=True)
                await self.document_repo.create(document_model)
            except AppBaseException:
                raise
            except:
                raise OperationFailureException(500, "Internal server error")
            
        return OperationOutput(id=None, request="create/all_documents", status=True)