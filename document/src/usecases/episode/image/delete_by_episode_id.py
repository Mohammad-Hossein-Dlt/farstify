...
class DeleteAllImages:
    ...
    async def execute(
        ...
        try:
            ...
            images: list[EpisodeImageModel] = await self.episode_image_repo.get_by_episode_id(episode_id)
            
            status = True if images else False
            for i in images:
                result = await self.storage_repo.delete_object(f"{episode.id}/" + "image/" + i.cover)
                if result:
                    await self.episode_image_repo.delete_by_id(i.id)
                else:
                    status = False
            
            return OperationOutput(id=episode_id, request="delete/all-episode-images", status=status)        
