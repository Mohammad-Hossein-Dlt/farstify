...
class ReorderImages:
    ...
    async def execute(
        ...
        try:
            images_list: list[DocumentImageModel] = await self.document_image_repo.get_images(document_id)
            for index, image_id in enumerate(image_ids):
                for image in images_list:
                    if str(image.id) == image_id:
                        image.order = index
                        await self.document_image_repo.update_image(image)

            return await self.document_image_repo.get_images(document_id)
