...
class UpdateImage:
    ...
    async def execute(
        ...
        try:
            image_model: DocumentImageModel = DocumentImageModel.model_validate(to_update, from_attributes=True)
            image: DocumentImageModel = await self.document_image_repo.get_image_by_id(to_update.id)
            document: DocumentModel = await self.document_repo.get_document_by_id(image.document_id)
            ...
            if all([file, file_name, file_size, content_type]):
                try:
                    ...
                    if result:
                        ...
                        image: DocumentImageModel = await self.document_image_repo.update_image(image_model)
                        await self.storage_repo.delete_object(f"{document.id}/" + pre_cover)
                except:
                    image.cover = pre_cover
                    image: DocumentImageModel = await self.document_image_repo.update_image(image)
