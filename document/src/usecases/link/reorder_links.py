...
class ReorderLinks:
    ...
    async def execute(
        ...
        try:
            links_list: list[DocumentLinkModel] = await self.document_link_repo.get_links(document_id)
            for index, links_id in enumerate(link_ids):
                for link in links_list:
                    if str(link.id) == links_id:
                        link.order = index
                        await self.document_link_repo.update_link(link)

            return await self.document_link_repo.get_links(document_id)        
