...
class UpdateLink:
    ...
    async def execute(
        self,
        to_update: UpdateLinkInput,
    ) -> DocumentLinkModel:
        
        try:
            link_model: DocumentLinkModel = DocumentLinkModel.model_validate(to_update, from_attributes=True)            
            return await self.document_link_repo.update_link(link_model)
