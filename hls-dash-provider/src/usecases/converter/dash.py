...
from src.infra.converter.dash import dash_command
    ...
    async def execute(
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp:
                ...
                command, m4s_path, mpd_path = dash_command(temp, "pipe:0", self.converter_params.bitrate)
                ...
                with open(mpd_path, "r", encoding="utf-8") as f:
                    content = f.read()
                    content = content.strip().replace(f"{m4s_path.parent}/", "").replace(f"{m4s_path.parent}\\", "")
                    
                converteds = await self.save_cache_usecase.execute(
                    f"convert:{object_path}",
                    {
                        self.converter_params.bitrate: content
                    },
                )
                
                if self.converter_params.number == len(converteds):
                    await self.broker_service.create_stream_file(object_name, Format.dash)
